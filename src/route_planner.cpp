...
// - Uses CalculateHValue to set the h-Value calculation.
// - For each node in current_node.neighbors, adds the neighbor to open_list and set the node's visited attribute to true.
...
// NextNode method is used to sort the open list and return the next node.
...
{
    std::sort(open_list.begin(), open_list.end(), [](const RouteModel::Node *a, RouteModel::Node *b) -> bool
...
// ConstructFinalPath method to returns the final path found from A* search.
// - This method should takes the current (final) node as an argument and iteratively follow the
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node)
{
    ...
    // Iterate throughs node until we reach the starting node
    ...
    // Once we found the starting node add it to the path and reverse the order of path_found
    path_found.push_back(*current_node);
...
void RoutePlanner::AStarSearch()
{
    ...
    // Set the current_node to the start node, set it as visited and get all its neighbors
    current_node = start_node;
    current_node->visited = true;
    AddNeighbors(current_node);

    // Check all nodes in the open list vector
    while (open_list.size() > 0)
    {
        current_node = NextNode();

        // Once we reach the end we can contruct the destination path
